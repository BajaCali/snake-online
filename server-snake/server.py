...
def str_map():
    out = [[[None] for i in range(window_size[0])] for j in range(window_size[1])]
    for y in range(len(playground)):
        for x in range(len(playground[y])):
            if playground[y][x] == Apple:
                out[y][x] = chr(2)
            elif playground[y][x] == None:
                out[y][x] = chr(0)
            else:
                for i in range(len(snakes)):
                    if playground[y][x] == snakes[i]:
                        out[y][x] = chr(101+i)
    return out

def send_msg(clients):
    out = {}
    out['y'] = str(y)
    out['x'] = str(x)
    out['snakes'] = len(snakes)
